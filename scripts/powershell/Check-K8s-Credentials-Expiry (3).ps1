...
.THRESHOLD POLICY
    The credential warning threshold is a STATIC CONSTANT defined by the
    $WarningThresholdDays parameter's default value (55 days). STIG mandates
    a 60-day hard limit; 55 days gives a 5-day lead time for remediation
    before STIG non-compliance kicks in.

    The unified monitoring pipeline does NOT expose this value as a pipeline
    parameter. Azure DevOps Server's parameter schema does not support
    type: int, and exposing it as a string invites silent typo-driven
    misconfigurations. To change the threshold, edit the default value of
    $WarningThresholdDays in this file.
...
.PARAMETER WarningThresholdDays
    Static default: 55 days (STIG 60-day hard limit; 5-day lead time).
    To change the threshold, edit this default value in the script - do
    NOT re-introduce a pipeline parameter for it.
#>
